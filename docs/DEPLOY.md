...
**Любое изменение `.env` требует рестарта**: переменные читаются процессом
только при старте — `systemctl restart hepfeed`. Для `chat_id` в Telegram
используйте **числовой ID чата** (узнать: написать боту @userinfobot);
@теги пользователей в `chat_id` не работают — только для публичных каналов.
