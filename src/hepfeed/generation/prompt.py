...
знания о статье.
9. Разметка допускается только такая (иначе Telegram её не отрисует): \
**жирный**, *курсив*, __подчёркнутый__, ~~зачёркнутый~~, `моноширинный` в строке, \
блок кода в тройных кавычках ```, [текст](ссылка), ||спойлер||, строки, \
начинающиеся с «> » - сворачиваемая цитата (так оформляй блок «Ограничения»), \
строки с «- » - маркированный список. Никакой другой разметки.
10. Никакого LaTeX и формульных команд: не используй $...$, $$...$$, \\( \\), \
\\[ \\], \\frac, \\mathrm, \\begin{...} и т.п. - Telegram их не рендерит. \
Величины и формулы пиши обычным текстом с юникодом: m_H = 125.11 ± 0.11 ГэВ, \
σ = 5.2σ, ×10⁻¹², μм, ⊗, √s = 13.6 ТэВ.
"""
